...
import { Settings as SettingsIcon, Database, Users, Bell, Palette, Globe, Save } from 'lucide-react'
  ...
  const saveSettings = async () => {
    ...
    try {
      ...
      for (const [key, value] of Object.entries(settings)) {
        if (!value && value !== '') continue // Skip undefined/null but allow empty string
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <label className="block text-gray-700 font-bold mb-2">
                      הודעה לתשלום (הנהלת חשבונות)
                    </label>
                    <textarea
                      className="input-field"
                      rows="4"
                      value={settings.whatsapp_payment_reminder}
                      onChange={(e) => updateSetting('whatsapp_payment_reminder', e.target.value)}
                      placeholder="שלום {customer_name}, נא להסדיר תשלום עבור הזמנה {order_number}."
                    />
                    <p className="text-sm text-gray-600 mt-2">
                      משתנים זמינים: <code className="bg-gray-100 px-2 py-1 rounded">{'{customer_name}'}</code>, <code className="bg-gray-100 px-2 py-1 rounded">{'{order_number}'}</code>
                    </p>
